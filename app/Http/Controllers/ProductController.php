    ...
    public function index()
{
    ...
    // اضافه به ProductController
    public function ajaxList(Request $request)
    {
        $query = \App\Models\Product::with('category')->where('is_active', 1);

        if ($request->filled('category_id')) {
            $query->where('category_id', $request->category_id);
        } else {
            // فقط محصولات با دسته‌بندی product
            $catIds = \App\Models\Category::where('category_type', 'product')->pluck('id');
